...
async function initDatabase() {
  try {
    ...
    try {
      ...
      logger.info('Columna titulo añadida a paginas', { context: 'db' });
    } catch (err) {
      const error = err as any;
      if (error.code === 'ER_DUP_FIELDNAME') {
        logger.info('La columna titulo ya existe en paginas', { context: 'db' });
      } else {
        logger.error('Error al crear columna titulo en paginas', { error: error.message, stack: error.stack, context: 'db' });
      }
    }
    try {
      await pool.query("ALTER TABLE paginas ADD COLUMN contenido TEXT NULL");
      logger.info('Columna contenido añadida a paginas', { context: 'db' });
    } catch (err) {
      const error = err as any;
      if (error.code === 'ER_DUP_FIELDNAME') {
        logger.info('La columna contenido ya existe en paginas', { context: 'db' });
      } else {
        logger.error('Error al crear columna contenido en paginas', { error: error.message, stack: error.stack, context: 'db' });
      }
    }
    try {
      await pool.query("ALTER TABLE paginas ADD COLUMN propietario INT DEFAULT 1");
      logger.info('Columna propietario añadida a paginas', { context: 'db' });
    } catch (err) {
      const error = err as any;
      if (error.code === 'ER_DUP_FIELDNAME') {
        logger.info('La columna propietario ya existe en paginas', { context: 'db' });
      } else {
        logger.error('Error al crear columna propietario en paginas', { error: error.message, stack: error.stack, context: 'db' });
      }
    }
    try {
      await pool.query("ALTER TABLE paginas ADD COLUMN oculto BOOLEAN DEFAULT 0");
      logger.info('Columna oculto añadida a paginas', { context: 'db' });
    } catch (err) {
      const error = err as any;
      if (error.code === 'ER_DUP_FIELDNAME') {
        logger.info('La columna oculto ya existe en paginas', { context: 'db' });
      } else {
        logger.error('Error al crear columna oculto en paginas', { error: error.message, stack: error.stack, context: 'db' });
      }
    }
    try {
      await pool.query("ALTER TABLE paginas ADD COLUMN descripcion VARCHAR(50) DEFAULT 'visible'");
      logger.info('Columna descripcion añadida a paginas', { context: 'db' });
    } catch (err) {
      const error = err as any;
      if (error.code === 'ER_DUP_FIELDNAME') {
        logger.info('La columna descripcion ya existe en paginas', { context: 'db' });
      } else {
        logger.error('Error al crear columna descripcion en paginas', { error: error.message, stack: error.stack, context: 'db' });
      }
    }
