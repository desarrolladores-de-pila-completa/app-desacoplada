...
async function initDatabase() {
    try {
      ...
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ...
        logger_1.default.info('Tabla verificada/creada correctamente', { table: 'invitados_global_chat', context: 'db' });
      ...
      FOREIGN KEY (receiver_id) REFERENCES users(id) ON DELETE CASCADE
        ...
        try {
            ...
            logger_1.default.info('Restricción de clave foránea eliminada del sender_id en private_messages', { context: 'db' });
        }
        catch (err) {
            const error = err;
            if (error.code === 'ER_CANT_DROP_FIELD_OR_KEY') {
                logger_1.default.info('La restricción de clave foránea ya fue eliminada o no existe en private_messages', { context: 'db' });
            }
            else {
                logger_1.default.error('Error al eliminar restricción de clave foránea en private_messages', { error: error.message, stack: error.stack, context: 'db' });
            }
        }
        logger_1.default.info('Tabla verificada/creada correctamente', { table: 'private_messages', context: 'db' });
    }
    catch (err) {
