...
class FeedRepository {
    async findAll(limit = 20, offset = 0) {
        const [rows] = await db_1.pool.query(`SELECT f.*, u.username, u.foto_perfil
       FROM feed f
    ...
    async createForUser(userId, username) {
        ...
        const [result] = await db_1.pool.query("INSERT INTO feed (user_id, mensaje, enlace) VALUES (?, ?, ?)", [userId, mensaje, enlace]);
