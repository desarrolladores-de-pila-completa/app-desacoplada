...
const UsernameUpdateService_1 = require("../src/services/UsernameUpdateService");
const ContentUpdateService_1 = require("../src/services/ContentUpdateService");
...
describe('UsernameUpdateService', () => {
    let usernameUpdateService;
    let mockContentUpdateService;
    ...
    const mockCacheInvalidationResult = {
        success: true,
    ...
    beforeEach(() => {
        ...
        // Configurar mocks
        ContentUpdateService_1.ContentUpdateService.mockImplementation(() => mockContentUpdateService);
        CacheInvalidationService_1.CacheInvalidationService.mockImplementation(() => mockCacheInvalidationService);
        db_1.pool = mockPool;
        // Crear instancia del servicio
        usernameUpdateService = new UsernameUpdateService_1.UsernameUpdateService();
        // Reset logs
        jest.clearAllMocks();
    });
    afterEach(() => {
    ...
    describe('Invalidación de caché', () => {
        it('debería invalidar caché correctamente', async () => {
            ...
            const result = await usernameUpdateService.updateUsername(options);
            // Assert
            expect(mockCacheInvalidationService.invalidateUserCache).toHaveBeenCalledWith(expect.objectContaining({
                oldUsername: 'oldusername',
                newUsername: 'newusername',
                userId: 'user-123',
                user: mockUser
    ...
    describe('UsernameUpdateService - Pruebas de Integración', () => {
        ...
        describe('Flujo completo de actualización con servicios reales', () => {
            ...
            it('debería manejar errores de servicios colaboradores correctamente', async () => {
                ...
                mockContentUpdateService.updateContentReferences.mockRejectedValue(new Error('Error de conexión a base de datos'));
                // Act
                const result = await usernameUpdateService.updateUsername(options);
                // Assert
                expect(result.success).toBe(false);
                expect(result.errors.some(error => error.includes('Error actualizando referencias de contenido'))).toBe(true);
                expect(result.contentUpdate).toBeUndefined();
            });
        });
        describe('Preview con servicios reales', () => {
            it('debería generar preview completo con datos reales', async () => {
                ...
                db_1.pool.query.mockResolvedValueOnce([[mockUser]]);
        ...
        describe('Escenarios de error realistas', () => {
            it('debería manejar errores de conexión a base de datos', async () => {
                ...
                db_1.pool.query.mockRejectedValue(new Error('Connection lost'));
                // Act & Assert
            ...
            it('debería continuar con proceso si creación de redirecciones falla parcialmente', async () => {
                ...
                // Assert
                expect(result.success).toBe(false);
                expect(result.errors.some(error => error.includes('Error creando redirecciones'))).toBe(true);
    ...
    describe('Creación de redirecciones', () => {
        it('debería crear redirecciones 301 correctamente', async () => {
            ...
            const result = await usernameUpdateService.updateUsername(options);
            // Assert
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO redirects'), expect.arrayContaining(['/oldusername', '/newusername']));
            expect(mockConnection.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO redirects'), expect.arrayContaining(['/pagina/oldusername', '/pagina/newusername']));
            expect(result.redirectsCreated).toBe(2);
        });
        it('debería omitir creación de redirecciones cuando skipRedirects es true', async () => {
            ...
            const result = await usernameUpdateService.updateUsername(options);
            // Assert
            expect(mockConnection.query).not.toHaveBeenCalledWith(expect.stringContaining('INSERT INTO redirects'), expect.any(Array));
            expect(result.redirectsCreated).toBe(0);
            expect(result.success).toBe(true);
        });
    });
    ...
    describe('Dry run mode', () => {
        it('debería ejecutar sin hacer cambios permanentes en modo dry run', async () => {
            ...
            // Assert
            expect(result.success).toBe(true);
            expect(mockContentUpdateService.updateContentReferences).toHaveBeenCalledWith(expect.objectContaining({ dryRun: true }));
            expect(mockCacheInvalidationService.invalidateUserCache).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ dryRun: true }));
    ...
    describe('Preview functionality', () => {
        ...
        it('debería marcar canProceed como false cuando hay errores de validación', async () => {
            ...
            ValidationService_1.ValidationService.validateUpdateUsername.mockReturnValue({
    ...
    describe('Métodos auxiliares', () => {
        ...
        describe('cleanupExpiredRedirects', () => {
            it('debería limpiar redirecciones expiradas', async () => {
                ...
                expect(mockPool.query).toHaveBeenCalledWith('DELETE FROM redirects WHERE expires_at < NOW()');
            });
        });
    });
});
//# sourceMappingURL=username-update.test.js.map