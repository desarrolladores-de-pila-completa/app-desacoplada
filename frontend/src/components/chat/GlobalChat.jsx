...
function GlobalChat() {
  ...
  console.log('GlobalChat: Ejecutando hooks de React Query inline');

  // Hook para mensajes globales (fallback, ya que se maneja por WebSocket)
  const { data: messages, isLoading, error } = useQuery({
    queryKey: ['globalChat', 50, 0],
    queryFn: async () => {
      // Rutas eliminadas, devolver array vacío
      return [];
    },
  });

  // Mutation para enviar mensaje global (fallback)
  const queryClient = useQueryClient();
  const sendMessageMutation = useMutation({
    mutationFn: async (messageData) => {
      // Rutas eliminadas, no hacer nada
      return { success: true };
    },
    onSuccess: () => {
  ...
  // Hook para mensajes privados (fallback)
  const { data: privateMessages, isLoading: privateLoading, error: privateError, refetch: refetchPrivateMessages } = useQuery({
    queryKey: ['privateChat', privateUserId && isPrivateChat ? privateUserId : null, 50, 0],
    queryFn: async () => {
      // Rutas eliminadas, devolver array vacío
      return [];
    },
    enabled: !!privateUserId && isPrivateChat,
  });

  // Mutation para enviar mensaje privado (fallback)
  const sendPrivateMessageMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['privateChat', privateUserId] });
    },
  });
