...
import usernameUpdateService from '../services/UsernameUpdateService';
...
function UsernameUpdateForm() {
  ...
  useEffect(() => {
    if (newUsername) {
      const validationResult = usernameUpdateService.validateUsername(newUsername);
  ...
  const loadStatistics = async () => {
    ...
    try {
      const result = await usernameUpdateService.getUpdateStatistics(user.id);
      if (result.success) {
  ...
  const handlePreview = async () => {
    ...
    try {
      const result = await usernameUpdateService.previewUsernameUpdate(user.id, newUsername);
      if (result.success) {
        setPreviewData(result.data);
        setShowPreview(true);
      } else {
        setError('Error al obtener preview: ' + result.message);
      }
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const result = await usernameUpdateService.updateUsername(user.id, newUsername);
      if (result.success) {
        ...
        // Recargar datos del usuario en el store
