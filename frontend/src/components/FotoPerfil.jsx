...
// Importar las nuevas utilidades de optimización de caché
import photoProfileImageCompressor from "../utils/PhotoProfileImageCompressor.js";
import photoProfileStorageManager from "../utils/PhotoProfileStorageManager.js";
import photoProfileSmartCleanup from "../utils/PhotoProfileSmartCleanup.js";
import photoProfileCacheOptimizer from "../utils/PhotoProfileCacheOptimizer.js";
import PhotoProfileStorageConfig from "../config/PhotoProfileStorageConfig.js";
...
const retryFetch = async (url, options = {}, maxRetries = 3, delay = 1000) => {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      logger.warn(`Intento ${i + 1} falló, reintentando...`, { status: response.status, url });
      await new Promise(resolve => setTimeout(resolve, delay * (i + 1)));
    } catch (error) {
      if (i === maxRetries - 1) throw error;
      logger.warn(`Error de red en intento ${i + 1}, reintentando...`, { error: error.message, url });
      await new Promise(resolve => setTimeout(resolve, delay * (i + 1)));
    }
  }
};
