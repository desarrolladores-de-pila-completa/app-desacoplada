...
const ContentRenderer = ({
  ...
  // Procesar contenido de manera simple
  const processed = content || '';
  const analysis = { type: 'text', elements: [], entities: [], length: processed.length, preview: processed.substring(0, 50) };
  const needsHTML = /<[^>]*>/.test(processed);
  const isSafe = true; // Asumir seguro para simplificar
  const isProcessing = false;
  const debug = { type: 'simple' };
