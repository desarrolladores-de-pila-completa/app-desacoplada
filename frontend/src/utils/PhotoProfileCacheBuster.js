// Mecanismo avanzado de "cache busting" para actualización inmediata sin caché temporal
class PhotoProfileCacheBuster {
  constructor() {
    ...
    this.logger = {
      info: (message, data) => {
        console.log(`[PhotoProfileCacheBuster] ${message}`, data || '');
      },
      error: (message, error) => {
        console.error(`[PhotoProfileCacheBuster] ${message}`, error);
      },
      warn: (message, data) => {
        console.warn(`[PhotoProfileCacheBuster] ${message}`, data || '');
      }
    };
  }
  ...
  generateBustUrls(userId, bustKey) {
    const baseUrl = `${window.API_URL || 'http://localhost:3000'}/api/auth/user/${userId}/foto`;
    const timestamp = Date.now();
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        const response = await fetch(urlInfo.url, {
          ...
          headers: {
            'Accept': 'image/*',
            'User-Agent': `PhotoProfileCacheBuster/1.0 (User:${userId})`,
            ...urlInfo.headers
          },
  ...
  async clearAllCaches(userId) {
    try {
      // 1. Limpiar service workers y cache storage
      if ('caches' in window) {
      ...
      // 2. Limpiar localStorage relacionado con fotos de perfil
      const keysToRemove = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && (
          key.includes('fotoPerfil') ||
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (key && key.includes('fotoPerfil')) {
      ...
      // 4. Forzar garbage collection de URLs de objetos
      this.forceObjectUrlCleanup();
  ...
  // Forzar limpieza de URLs de objetos
  forceObjectUrlCleanup() {
    // Esta es una técnica avanzada para forzar cleanup de memoria
    if (typeof window !== 'undefined' && window.gc) {
      try {
        window.gc();
      } catch (error) {
        // gc no disponible en todos los navegadores
      }
    }
  }
